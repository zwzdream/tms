					...
					<table class="gridtable" style="width: 100%; text-align: center;">
						<tr>
							<c:if test="${page.hasPreviousPage}">
								<td><a href="#"
									onclick="ajaxContent('/JD/all/list','pageNum=${page.prePage}&pageSize=${page.pageSize}');">Previous</a></td>
							</c:if>
							<c:forEach items="${page.navigatepageNums}" var="nav">
								...
								<c:if test="${nav != page.pageNum}">
									<td><a href="#"
										onclick="ajaxContent('/JD/all/list','pageNum=${nav}&pageSize=${page.pageSize}')">${nav}</a></td>
								</c:if>
							</c:forEach>
							<c:if test="${page.hasNextPage}">
								<td><a href="#"
									onclick="ajaxContent('/JD/all/list','pageNum=${page.nextPage}&pageSize=${page.pageSize}');">Next</a></td>
							</c:if>
						</tr>
					</table>
				</div>
